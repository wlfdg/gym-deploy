...
function Attendance() {
  ...
  const timeIn = async (member) => {
    try {
      const res = await api.post("/attendance/timein", { member_id: member.id });
      clearCache();
  ...
  const timeOut = async (memberId) => {
    try {
      const res = await api.post("/attendance/timeout", { member_id: memberId });
      clearCache();
      showMsg(res.data.message);
      fetchAttendance();
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {data.records.filter(r => !r.time_out).map(r => (
                  <div key={r.id} style={{
                    ...
                    <button className="btn btn-danger btn-sm" style={{ padding: "4px 10px", fontSize: 10 }}
                      onClick={() => timeOut(r.member_id)}>Out</button>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>#</th><th>Member</th><th>Time In</th><th>Time Out</th><th>Duration</th><th>Status</th></tr>
              </thead>
              <tbody>
